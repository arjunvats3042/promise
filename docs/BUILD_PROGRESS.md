...
- **Current phase:** Latest Dev (Batch 21) — MacBook-Style Concierge Window Zoom Animation, 1-Tap Action Chips Deep-Linking, 0ms Instant Response Cache, Typewriter Stream Rendering, Theme Switcher Crash Fix, and Strict Knowledge Guardrails **COMPLETE**
- **Overall status:**
...
- **Last completed milestone:** Batch 21 — MacBook-Style Concierge Window Zoom Animation, 1-Tap Action Chips Deep-Linking, 0ms Instant Response Cache, Typewriter Stream Rendering, Theme Switcher Crash Fix, and Strict Knowledge Guardrails (2026-09-01)
...
- Batch 20 Full-Stack Synchronization Hardening, Dynamic Roadmap Wallpaper Engine, Offline Outbox Fallbacks & Real-Time Chat Ordering: **COMPLETE**
- Batch 21 MacBook-Style Concierge Window Zoom Animation, 1-Tap Action Chips Deep-Linking, 0ms Instant Response Cache, Typewriter Stream Rendering, Theme Switcher Crash Fix, and Strict Knowledge Guardrails: **COMPLETE** (Latest Dev)
...
- **2026-09-01:** Batch 21 MacBook-Style Concierge Window Zoom Animation, 1-Tap Action Chips Deep-Linking, 0ms Instant Response Cache, Typewriter Stream Rendering, Theme Switcher Crash Fix, and Strict Knowledge Guardrails **COMPLETE**.
  - **MacBook-Style Exact-Origin Zoom Window**: Rewrote `PromiseSupportChatSheet` to bloom directly out of `FloatingPromiseConciergeOrb` screen coordinates `(originX, originY)` using spring physics (`Spring.DampingRatioLowBouncy`, `Spring.StiffnessMediumLow`), smoothly expanding from `scale(0.12f)` to `scale(1.0f)` with frosted backdrop fade, and collapsing cleanly back into the orb on close, scrim tap, or system back button.
  - **1-Tap Action Deep-Link Badges**: Attached `SupportActionChip` pills (`CREATE_GOAL`, `CREATE_COMMITMENT`, `OPEN_THEME`, `OPEN_SHARED_GOALS`, `OPEN_PROFILE`, `OPEN_VOICE_CAPTURE`) below concierge answers in both AI prompt schema and heuristic knowledge base, wiring direct tab switches and creation flows in `MainShell.kt`.
  - **0ms Instant Response Resolution**: Added local knowledge cache in `PromiseSupportViewModel` allowing common suggestion chips and how-to queries to resolve in 0ms without network latency or loading spinners.
  - **Smooth Typewriter Text Animation**: Implemented character-by-character fluid streaming text reveal in `AssistantChatCard` with tap-to-skip gesture and `rememberReduceMotion()` support.
  - **Theme Switcher Crash Fix**: Eliminated `ClassCastException` on theme change by implementing `tailrec fun Context.findActivity(): Activity?` unwrapper in `Theme.kt`, protecting Glance widget theme broadcast with `runCatching` in `PromiseWidgetUpdater.kt`, and adding `@param:ApplicationContext` in `SharedPrefsThemeStore.kt`.
  - **Calendar History Bounds & Goal Detail Layout**: Locked clicks on dates before `goal.startDate` and future dates in `PromiseHistoryCalendar.kt`, and moved the Recent Activity card to the bottom of `GoalDetailScreen.kt`.
  - **Strict Knowledge & Off-Topic Guardrails**: Enhanced prompt v1.0.0 and `_heuristic_support_bot` to provide concise, 4-step direct account deletion guides and polite app concierge redirects for non-app questions or profanities.
  - **Verification**: Executed 50+ adversarial/intent test suite and verified `./gradlew assembleDebug` (BUILD SUCCESSFUL, 0 warnings, 0 errors).
